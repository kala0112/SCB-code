...
lines = [line.rstrip() for line in open('/home/ec2-user/scb-migration/blue.txt')]
...
def install_agent(args, installation_token):
    ...
    # Fetch the CloudEndure machine ID in order monitor the replication
    # progress and launch the target server
    print ('Getting machine id...')
    for project in projects:
        project_id = project['id']

        machines_resp = session.get(
            url=HOST +
            endpoint +
            'projects/' +
            project_id +
        ...
        machine_id = [m['id'] for m in machines if args.agentname.lower(
        ) == m['sourceProperties']['name'].lower()]
...
def wait_for_replication(args, machine_id, project_id):
    ...
    while True:
        ...
        while True:
            try:
                ...
                replication_status = json.loads(
                    machine_resp.content)['replicationStatus']
                break
        ...
        while True:
            try:
                replicated_storage_bytes = json.loads(
                    machine_resp.content)['replicationInfo']['replicatedStorageBytes']
                total_storage_bytes = json.loads(
                    machine_resp.content)['replicationInfo']['totalStorageBytes']
                break
            except BaseException:
                print ("Replication has not started. Waiting...")
                time.sleep(120)
        ...
        while True:
            try:
                last_consistency = json.loads(machine_resp.content)[
                    'replicationInfo']['lastConsistencyDateTime']
                backlog = json.loads(machine_resp.content)[
...
def login(args):
    ...
    resp = session.post(url=HOST + endpoint + 'login',
